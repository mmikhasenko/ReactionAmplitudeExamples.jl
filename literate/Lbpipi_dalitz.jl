...
# ╔═╡ dbf18719-d598-4e91-88a7-99dfc6195bb7
begin
...
# ╔═╡ 0e7005a2-1a86-452b-8e67-077c6a1b09b6
theme(:boxed)

# ╔═╡ 4de2fa76-0b83-4cae-bc58-ee1589e86136
begin
	const mΛb = 5.61960
	const mπ = 0.14
...
# ╔═╡ 1efe516c-3167-4134-b32b-b233000c8a3d
jp0 = SpinParity("1/2+")

# ╔═╡ 3377a0be-2aea-4d71-bb97-2f0a497dd371
tbs = ThreeBodySystem(
	ThreeBodyMasses(mπ,mΛb,mπ; m0=mΛb2S),
	ThreeBodySpins(0, 1, 0; two_h0=jp0.two_j)) #, ))  # 1/2+ 0- 0- 1/2+

# ╔═╡ a6d45bfe-955a-40ed-9679-209ba7368da5
Ps = ThreeBodyParities('-','+','-'; P0=jp0.p)

# ╔═╡ 2039c9ec-38cd-4a6c-b613-9ee5e3c8acd3
md"""
## Decay channels
"""

# ╔═╡ 25568759-1053-433e-9d1b-0696a4689a00
dc_Σb = [DecayChainLS(;
	k,
	Xlineshape=BreitWigner(mΣb,ΓΣb),
	tbs,
	jp=SpinParity("1/2+"),
	Ps) for k=[1,3]]

# ╔═╡ ff39aef4-bd48-4401-9391-15991886ffe2
dc_Σb_x = [DecayChainLS(;
	k,
	Xlineshape=BreitWigner(mΣb_x,ΓΣb_x),
	tbs,
	Ps,
	jp = "3/2+") for k in [1,3]]

# ╔═╡ ee45557f-20e1-4ae0-8c57-ee36b622cb8e
dc_f0 = DecayChainLS(;
	k=2,
	Xlineshape=BreitWigner(0.3,1.5),
	tbs,
	Ps,
	jp = "0+")

# ╔═╡ 650e7761-4ab6-45b4-adf9-92d18c872266
model = ThreeBodyDecay(
	"name" .=> zip((1.0,1.0,2.0,2.0, 5.1),(dc_Σb...,dc_Σb_x...,dc_f0)))

# ╔═╡ e922dbfe-1417-44e7-8a26-d37ddf4193c9
md"""
## Intensity
"""

# ╔═╡ a2d13606-a2bf-4d12-aa97-e8dca0354d4f
rp = randomPoint(tbs.ms)

# ╔═╡ 96ec93a0-d521-4cd8-8606-e8088b5cfe9a
unpolarized_intensity(model[5], rp)

# ╔═╡ e158239c-3448-4577-87f5-74641530bdb8
full_a(σs,two_λs) = sum(amplitude(σs,two_λs, ch) for ch in [dc_Σb1, dc_Σb_x1, dc_Σb3, dc_Σb_x3])

# ╔═╡ 12f20563-1236-4303-af73-459a029deef8
full_a(dpp) = full_a(dpp.σs, dpp.two_λs)

# ╔═╡ 5d33bc08-f27c-4d1e-81c8-5bea2e38c22a
plot(tbs.ms, Base.Fix1(unpolarized_intensity, model);
	iσx=2, iσy=1, c=:viridis, title="\$J^P(\\Lambda_b^0)= $(tbs.two_js[4])/2^$(Ps[4])\$", size=(500,500),
	xlab="\$m(\\pi^+\\pi^-)\$", ylab="\$m(\\Lambda_b^0\\pi^-)\$")
...
manifest_format = "2.0"
project_hash = "43ecc108a01ddac82a74b4637cd8bd236bff281f"
