...
function double_regge(y1, y2; pars)
	@unpack α1, α2, s1, s2, s12 = pars
	return y1^(-1-α1) / (1-y1*s1) *
		y2^(-1-α2) / (1-y2*s2) *
		1 / (1-y1*y2*s12)
end
...
# ╔═╡ 866cdc98-3602-47a4-94b7-87321724b4e4
A(s1) = A(; pars = (; s1, s2=s2+iϵ, s12=s12+iϵ, α1, α2))

# ╔═╡ dd90d59e-49d7-4efe-bc85-05876a7a383f
calculations = let
	s1v = range(-1, 2.2, 30)
	calv = map(s1v) do s1
		A(s1+1e-7im)
	end
	DataFrame(s1 = s1v, A = calv)
end

# ╔═╡ 4e8bbe98-0355-498d-a63c-aca2cb327241
begin
	plot()
	plot!(calculations.s1, calculations.A .|> real, lab="Re")
	plot!(calculations.s1, calculations.A .|> imag, lab="Im")
	plot!()
end
...
julia_version = "1.9.2"
manifest_format = "2.0"
project_hash = "25247ef14fbb445aaecb615c04d7069ede3b95cc"
