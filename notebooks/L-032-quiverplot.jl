...
# ╔═╡ 9bc5908c-e1c1-4764-a20a-f14065c43eeb
md"""
# Quiver plot (vector field)

This notebook demonstrates how to plot a vector field on 2d manifold.
It uses `Plots.quiver` recipe.

Example here is related to the [$\Lambda_c^+$ polarimetry paper](https://inspirehep.net/literature/2623821).
"""
...
begin
	mass_in_GeV(name) = convert(Float64, Particle(name).mass.value / 1u"GeV*c^-2")
    const mΛc = mass_in_GeV("Lambda(c)+")
    const mp = mass_in_GeV("p")
    const mπ = mass_in_GeV("pi+")
    const mK = mass_in_GeV("K+")
end;
...
let length = 15
	...
	# 
    plot(layout = grid(1, 2), size = (700, 350), bottom_margin = 3mm)
    # 
    ...
    plot!(sp = 1, border12(ms), lc = :black)
    # # 
    quiver!(sp = 2, getindex.(xyv, 1), getindex.(xyv, 2),
        quiver = (x, y) -> field((x, y); wr = wr(2, 1, 0)) ./ length,
        c = 3, arrow = (:closed,),
        aspectratio = diff([lims1(ms)...])[1] / diff([lims2(ms)...])[1])
    plot!(sp = 2, border12(ms), lc = :black)
	# 
	plot!(title = [L"\alpha(\Delta)" L"\alpha(\Lambda)"],
        xlab = L"m(\pi K)\,\,(\mathrm{GeV})", ylab = L"m(pK)\,\,(\mathrm{GeV})")
end
...
manifest_format = "2.0"
project_hash = "55dc48727deec5ea137eb3c5305cf3a6f23fd5e4"
