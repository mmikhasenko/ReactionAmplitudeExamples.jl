...
θ,z = @syms θ::positive=>"θ" z::positive

# ╔═╡ 009bc6a0-dcde-4c47-a403-82a86b1fb7f9
clg(j1,m1,j2,m2,j,m) = sympy.physics.wigner.clebsch_gordan(Sym(j1),j2,j,m1,m2,m);

# ╔═╡ ecc592ce-0010-4188-bd8c-6fc83ed53128
dl(L,J,λ,ν) = sqrt(2L+Sym(1)) / sqrt(2J+Sym(1)) * sqrt(Sym(2)) *
	sympy.physics.quantum.spin.WignerD(Sym(1)/2,λ,ν,0,θ,0).doit() * clg(L,0,Sym(1)/2,λ,J,λ)
...
manifest_format = "2.0"
project_hash = "c3c38d4cb8eadddef7d82cae8011d4526eed5e7c"
