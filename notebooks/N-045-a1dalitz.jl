...
# ╔═╡ 381e2598-1207-415a-970a-e277ac333ee4
md"""
# Dalitz Plot for $a_1(1260)$

A plot for JPAC review ([inspirehep](https://inspirehep.net/literature/1997164)).
"""
...
ρ_BW = BreitWigner(; m=mρ, Γ=Γρ, ma=mπ, mb=mπ, l=1, d=1.5)

# ╔═╡ e0e86f1e-c4d9-4558-80b7-42979f62ff7f
begin
    plot(σs -> abs2(ρ_BW(σs[1]) + ρ_BW(σs[2])), ms, aspectratio = 1)
    plot!(border12(ms), l = (4, :black), lab = "", aspectratio = 1)
end
...
const dc = [DecayChain(; k, Xlineshape = ρ_BW, two_j = 1 |> x2,
    Hij = RecouplingLS((1, 0) |> x2),
    HRk = RecouplingLS((0, 1) |> x2),
    tbs) for k in 1:2]
...
manifest_format = "2.0"
project_hash = "150ff457fcc2be866f8531ef009cb83ae3c9ba42"
