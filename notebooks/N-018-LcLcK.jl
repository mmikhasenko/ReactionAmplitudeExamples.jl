...
# ╔═╡ cb815c9a-f2f9-4c65-812d-7f827805e138
begin
	const mK  =  493.677 # MeV
	const mB  = 5279.32  # MeV
	const mΛc = 2286.46  # MeV
end
...
begin
	...
	q(s) = s > (mΛc+mK)^2 ? sqrt(λ(s,mΛc^2,mK^2)/(4s)) : 0.0
	p(s) = s < (mB-mΛc)^2 ? sqrt(λ(mB^2,s,mΛc^2))/(2mB) : 0.0
	# 
	ρ(s) = q(s)/sqrt(s)
end;
...
begin
	...
	Fl²(z,l::Int) = l==0 ? F₀²(z) :
		(l==1 ? F₁²(z) :
			(l==2 ? F₂²(z) : error("l ≤ 2 coded")))
end;

# ╔═╡ 857459e4-278a-4d59-ad4f-69846d2a6b45
gᵢ²(m,Γ,l) = (m^2*Γ) / (q(m^2)*Fl²(q(m^2)*r₀,l))

# ╔═╡ 5c054a5c-bd19-410e-b216-59461ab138f1
begin
	const g₁ = sqrt(gᵢ²(m₁,Γ₁,l₀)) # sqrt(36.4 * m₁)
	const g₂ = sqrt(gᵢ²(m₂,Γ₂,l₀)) # sqrt(118.2 * m₂)
end

# ╔═╡ 07de28a0-9931-4c07-b6c3-80efb6ea9b42
md"""
### Couplings from the table 
"""
...
# ╔═╡ 83bd360d-2a81-4bd1-9ff2-57875f5877f2
K(s) = g₁^2 / (m₁^2-s) + g₂^2 / (m₂^2-s)

# ╔═╡ ead380cc-2844-4087-8efb-08dcb7c67f95
P(s) = c₁*g₁ / (m₁^2-s) + c₂*g₂ / (m₂^2-s)

# ╔═╡ efa97122-af5d-4d58-ba51-945cc17850ef
Â(s) = P(s)/(1-1im*ρ(s)*Fl²(q(s)*r₀,l₀)*K(s))

# ╔═╡ 0c74d418-c1b4-41c6-b5a7-bdde74f88e8d
N(s) = p(s)*q(s)*abs2(Â(s)) * Fl²(q(s)*r₀,l₀) * Fl²(p(s)*r₀,L₀)

# ╔═╡ af56996c-7c11-474a-9497-843c45cbecfb
md"""
### Cross-check spectrum
"""

# ╔═╡ 84c6391e-b445-4100-b6b0-03ad725e22c4
plot(e->N(e^2), xlims..., lab="P-vector")
...
julia_version = "1.9.3"
manifest_format = "2.0"
project_hash = "906b7c45d5af153ed17caf15cd6385c36a327e3d"
