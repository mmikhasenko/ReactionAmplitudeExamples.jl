...
# ╔═╡ dab26a85-40b6-4350-879d-4057b0234ba3
md"""
# $a_1(1260)$ and $a_1(1420)$ pseudoexperiments

The notebook creates a three-body decay model with varied $s$, and samples toys using the model.
"""

# ╔═╡ 47666396-b2dd-40c7-afe2-02fa25591963
theme(:boxed, lab = "", colorbar = false)

# ╔═╡ 3e672231-c34c-4c99-8d68-2802607a20cc
function Osym(dc::T, σs, λ::Int) where T <: DecayChain
    dc.k == 1 && return amplitude(dc, σs, ThreeBodySpins(0, 0, 0; h0=λ))
    dc.k == 2 && return amplitude(dc, σs, ThreeBodySpins(0, 0, 0; h0=λ)) +
                        minusone()^div(dc.two_j, 2) *
                        amplitude(DecayChain(dc, k = 3), σs, ThreeBodySpins(0, 0, 0; h0=λ))
...
begin
    ...
    function (x::X3b)(σ::Float64)
        ...
        ff² = BlattWeisskopf{L}(r0)(q)
        return q^L * sqrt(ff²) * lineshape(σ)
    end
    # 
    @with_kw struct Wave{T,L} <: ThreeBodyDecays.AbstractDecayChain
		lineshape3b::T
		j0::Int
		P0::Char
		k::Int
    ...
    function wave2decaychain(wave::Wave, s::Float64)
        ...
        m0 = sqrt(s)
        _ms = ms(m0)
        tbs = ThreeBodySystem(; ms = _ms, two_js = ThreeBodySpins(0, 0, 0; h0=j0))
        Ps = ThreeBodyParities('-', '-', '-'; P0)
        d = DecayChainLS(; k=2, Xlineshape=identity,
			jp = SpinParity(2j, parity), Ps, tbs)
        Xlineshape = X3b(; lineshape, m0, mk = _ms[k], L = div(d.HRk.two_ls[1], 2))
        return DecayChainLS(; k, Xlineshape, jp = SpinParity(2j, parity), Ps, tbs)
    end
    function ThreeBodyDecays.amplitude(wave::Wave, s::Float64, σs, λ::Int)
        @unpack lineshape3b = wave
        dX = wave2decaychain(wave, s)
        return Osym(dX, σs, λ) * lineshape3b(s)
    end
end

# ╔═╡ ceb3339d-6fbd-4643-8d46-da2abd6e1af7
begin
    BW_Kˣ = BreitWigner(m = mKˣ, Γ = ΓKˣ, ma = mK, mb = mπ, l = 1, d=1.5)
    BW_f₀ = BreitWigner(m = mf₀, Γ = Γf₀, ma = mK, mb = mK, l = 0, d=1.5)
end;

# ╔═╡ 1aa2004e-0e5f-4a6e-9aaf-141d1ac51139
function intensity(x::X3b, σ::Float64)
    @unpack lineshape, m0, mk, mk = x
    @unpack ma, mb = lineshape
    sqrtσ = sqrt(σ)
    q = sqrtKallenFact(m0, sqrtσ, mk) / (2m0)
    p = sqrtKallenFact(sqrtσ, ma, mb) / (2sqrtσ)
    return abs2(lineshape(σ)) * p * q / sqrtσ
end

# ╔═╡ 9b7ec391-7f41-40e7-b6ea-6176cc86fdfe
begin
    BW_a₁1260 = BreitWigner(m = 1.26, Γ = 0.5, ma = 2mπ, mb = mπ, l = 0, d=3.0)
    BW_a₁1420 = BreitWigner(m = 1.42, Γ = 0.15, ma = 2mK, mb = mπ, l = 1, d=3.0)
end;

# ╔═╡ 67015bfb-b52f-4868-a4f4-c55904cfdde5
plot(ylab = L"|A|^2", xlab = L"m(KK\pi)\,\,(\mathrm{GeV})",
    plot(m -> abs2(BW_a₁1260(m^2)), 2mK, 2.5),
    plot(m -> abs2(BW_a₁1420(m^2)), 2mK, 2.5))
...
const waves = [
    Wave(lineshape3b = BW_a₁1260, j0 = 1, P0 = '+', k = 2, lineshape = BW_Kˣ, j = 1, parity = '-')
    Wave(lineshape3b = BW_a₁1420, j0 = 1, P0 = '+', k = 1, lineshape = BW_f₀, j = 0, parity = '+')
];
...
manifest_format = "2.0"
project_hash = "61a67193e2b53056bcb683da1189e27097f409e5"
