...
Prompt production of $\Omega_c^{**0}$.
Question is can we learn anything about $\Omega_c^{**0}$ quantum numbers by analysing the $\Xi_c^+$ decays.
"""
...
# ╔═╡ 217977d7-a1bb-47cd-a0fa-291a27491d47
@syms α β x
...
begin
	...
	dl(L,J,λ,ν) = sqrt(2L+Sym(1)) / sqrt(2J+Sym(1)) * sqrt(Sym(2)) *
		WignerD(J,λ,ν,0,θ,0).doit() * clg(L,0,Sym(1)/2,λ,J,λ)
end
...
Unpolarized intensity for the in decay Ωcˣˣ → Ξc K decay. Parameters:

- `J`, and `L` are the spin of Ωcˣˣ resonance, and orbital angular momentum.
- `α`, and `β` are the asymmetry parameters, of the decay, and the production.
"""
function intensity(L,J;α,β)
	production_pol(two_ν) = (1+two_ν*α)/Sym(2) # diagonal matrix
	decay_pol(two_λ) = (1+two_λ*β)/Sym(2) # diagonal matrix
	# 
	sympy.expand_trig(simplify(
		sum(
			dl(L,J,Sym(two_λ)/2,Sym(two_ν)/2) *
			dl(L,J,Sym(two_λ)/2,Sym(two_ν)/2) * 
		# 
		production_pol(two_λ*β) * 
		decay_pol(two_ν)
		# 
			for two_λ in -1:2:1,
				two_ν in -1:2:1)))
...
angular_funcs = map(two_JPs) do two_JP
	@unpack L,J = LJ(two_JP)
	intensity(L, J; α=α, β=1)
end

# ╔═╡ 84201724-b9a9-45f5-8d49-e0301dc20afe
fs = map(angular_funcs) do f
	f.subs(θ, acos(x)) |> sympy.trigsimp |> 
		y->lambdify(y, (x,α))
end

# ╔═╡ d239d2c6-7537-4c27-b500-02309fc21743
let
	plot()
	for f in fs
		plot!(x->f(x,0.5), -1, 1)
...
manifest_format = "2.0"
project_hash = "31ec7532d93ec774ad5aa835528f7d8c83c4b4ae"
